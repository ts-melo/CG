function main() {
    ...
    const n = 101; // Number of segments for drawing a petal
    let angle = 0; // Initial angle for rotation

    function animate() {
        gl.clear(gl.COLOR_BUFFER_BIT);
        angle += 0.01; // Increment the rotation angle for animation

        drawFlower(angle);
        requestAnimationFrame(animate);
    }

    function drawFlower(rotationAngle) {


    //caule
    gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer);
    setRectangleVertices(gl, -0.0, -1, 0.02, 1);
    ...
    setCircleVertices(gl, 0.1, -0.5, n, 0.08);
    gl.vertexAttribPointer(positionLocation, 2, gl.FLOAT, false, 0, 0);
    gl.enableVertexAttribArray(positionLocation);
    ...
    setTriangleVertices(gl, 0.0, -0.4, 0.0, -0.4, 0.0, -0.58); // Define triangle vertices
    gl.vertexAttribPointer(positionLocation, 2, gl.FLOAT, false, 0, 0);
    gl.enableVertexAttribArray(positionLocation);
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, trianglePositionBuffer);
    setTriangleVertices(gl, 0.3, -0.5, 0.1, -0.42, 0.1, -0.58); // Define triangle vertices
    gl.vertexAttribPointer(positionLocation, 2, gl.FLOAT, false, 0, 0);
    gl.enableVertexAttribArray(positionLocation);
    ...
    const radius = 0.3 - 0.1;  // Distance from the center to the petal

    for (let i = 0; i < numPetals; i++) {
      const petalAngle = (i * 2 * Math.PI / numPetals) + rotationAngle;

      // Compute petal position using rotation
      const x = radius * Math.cos(petalAngle);
      const y = radius * Math.sin(petalAngle);

      gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer);
      setPetalVertices(gl, x, y, n);
      gl.vertexAttribPointer(positionLocation, 2, gl.FLOAT, false, 0, 0);

      gl.bindBuffer(gl.ARRAY_BUFFER, colorBuffer);
      setPetalColor(gl, n, [1, 0, 0]); // Red color for the petal
      gl.vertexAttribPointer(colorLocation, 3, gl.FLOAT, false, 0, 0);
    ...
    // miolo
    gl.bindBuffer(gl.ARRAY_BUFFER, circlePositionBuffer);
    setCircleVertices(gl, 0, 0, n, 0.15);
    gl.vertexAttribPointer(positionLocation, 2, gl.FLOAT, false, 0, 0);
    gl.enableVertexAttribArray(positionLocation);
...
function setPetalColor(gl, segments, color) {
  const colors = [];

  // Create colors for each vertex
  for (let i = 0; i <= segments + 1; i++) {
      colors.push(...color); // Add RGB values (e.g., [1, 0, 0] for red)
  }

  // Bind and set data for the buffer
  gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(colors), gl.STATIC_DRAW);
}
function setPetalVertices(gl, offsetX, offsetY, segments) {
  const vertices = [offsetX, offsetY]; // Start at the center (offset position)

  // Define the petal shape using an arc (half-oval)
  const petalWidth = 0.2; // Width of the petal
  const petalHeight = 0.2; // Height of the petal

  for (let i = 0; i <= segments; i++) {
      const angle = (2 * Math.PI * i) / segments; // From 0 to PI for a half-circle
      const x = offsetX + petalWidth * Math.cos(angle);
      const y = offsetY + petalHeight * Math.sin(angle);
      vertices.push(x, y);
  }
